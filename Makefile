...
unit: ## run unit tests
	python3 -m pytest -vvl tests/unit/ --showlocals

integration: ## run integration tests
	python3 -m pytest -vvl --setup-show -vvl tests/integration/ --showlocals

run: ## run project
	uvicorn $(PROJECT_NAME).app:app --reload
...
run-docker-debug: build-docker ## run debug with docker on port 8000
	docker run --rm -it -v "${PWD}:/app" -p 8000:8000 --rm $(PROJECT_NAME):test uvicorn --host 0.0.0.0 --port 8000 --reload $(PROJECT_NAME).api:app
